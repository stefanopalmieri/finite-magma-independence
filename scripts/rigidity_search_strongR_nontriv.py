...
AND classifier-nontriviality on core.

Nontriviality axiom: there exists a classifier tau whose image on core has
>= 2 distinct values. Equivalently, some x1, x2 in core have tau.x1 != tau.x2.

This rules out the N=6 counterexample (classifier row on core is all z1)
and the N=7 counterexample (classifier row on core is all z2). Whether it
closes the gap at N=8 (which had four non-trivial classifiers) is what this
experiment tests.

Usage: python3 rigidity_search_strongR_nontriv.py <N>
"""
...
def check_D_nontriv(T, N, CORE):
    ...
    if not non_classifiers:
        return False, None
    # Need a tau in core with tau.x in {0,1} for all x AND |image on core| >= 2
    for tau in CORE:
...
def is_rdh_strong_nontriv(T, N, CORE):
    ...
    if not okD: return False, "D + nontriv"
...
def build_solver(N):
    ...
    # D + non-trivial classifier on core.
    is_cls = {}
    for y in CORE:
        ...
        is_cls[y] = all_in
    s.add(Or(*[Not(is_cls[y]) for y in CORE]))  # non-classifier exists
    # tau: classifier (all x land in {0,1}) AND non-trivial on core (|img on core| >= 2)
    tau_cases = []
    for tv in CORE:
        is_classifier_full = And(*[Or(T[tv][x] == 0, T[tv][x] == 1)
                                    for x in range(N)])
        nontriv_on_core = Or(*[T[tv][x1] != T[tv][x2]
                                for x1, x2 in itertools.combinations(CORE, 2)])
        tau_cases.append(And(is_classifier_full, nontriv_on_core))
    s.add(Or(*tau_cases))
...
def run(N):
    CORE = list(range(2, N))
    JSON_PATH = os.path.join(SCRIPT_DIR, f"nonrigid_rdh_N{N}_strongR_nontriv.json")
    TXT_PATH = os.path.join(SCRIPT_DIR, f"rigidity_search_N{N}_strongR_nontriv_result.txt")

    t0 = time.time()
    print(f"=== Z3: non-rigid R+D+H at N={N}, strong R + nontrivial classifier ===")
    ...
    if result == sat:
        ...
        ok, reason = is_rdh_strong_nontriv(table, N, CORE)
        print(f"\nVerification: R+D+H + strong-R + nontrivial-tau: {ok}, reason={reason}")
        ...
        data = {
            ...
            "constraint": "strong R (s != r) + nontrivial classifier on core",
        }
        ...
        print(f"UNSAT: every R+D+H + strong-R + nontrivial-tau magma at N={N} is role-rigid.")
        with open(TXT_PATH, "w") as f:
            f.write(f"UNSAT at N={N}, strong R + nontrivial classifier on core\n")
