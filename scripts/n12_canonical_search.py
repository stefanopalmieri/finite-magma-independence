"""
N=12 canonical-witness search.

Tests whether the canonical-witness program (S+D+C + indicator-style
classifiers + self-symmetric automorphism) is SAT at N=12. This is the
first step toward integrating with Ψ-Lisp's axiom system: if SAT at
N=12, the canonical-witness program at least *coexists* with Ψ-Lisp's
preferred working size; if UNSAT, the principle doesn't scale that
far.

The full synthesis (Ψ-Lisp axioms + canonical-witness) would require
importing the axiom encoding from
~/Desktop/DistinctionStructures/ds_search/stacking_analysis.py; that's
a substantial integration and not done here.
"""
...
def query(label, constrain_R, time_budget=1800):
    ...
    print(f"=== {label}: N={N} S+D+C + indicator + self-symmetric, R={constrain_R} ===", flush=True)
    t0 = time.time()
    res = s.check()
    dt = time.time() - t0
    if res == unsat:
    ...
    if res == sat:
        ...
        print(f"  [{dt:.1f}s] SAT — N={N} canonical witness found.")
        for row in table:
            print(f"    {row}")
        print(f"  σ = {sigma_val}")
        print(f"  ρ = {rho_val}; sR = {sR_val}, rR = {rR_val}")
        # classify role-shape
        cls = [y for y in CORE if all(table[y][x] in (0, 1) for x in CORE)]
        ncl = [y for y in CORE if all(table[y][x] not in (0, 1) for x in CORE)]
        print(f"  Role-shape: |classifiers| = {len(cls)}, |non-classifiers| = {len(ncl)}")
        return True, {"table": table, "sigma": sigma_val, "rho": rho_val,
                       "sR": sR_val, "rR": rR_val,
                       "cls_count": len(cls), "ncl_count": len(ncl)}
    print(f"  [{dt:.1f}s] UNKNOWN/timeout")
    return None, None


def main():
    print(f"N=12 canonical-witness search (no Ψ-Lisp axioms; just S+D+C + indicator + self-symmetric)")
    print()
    results = {}
    for R in (None, "strong", "weak"):
        ...
        print()
    print("=== Summary ===")
    if any(v["sat"] for v in results.values() if v["sat"]):
        regimes = [k for k, v in results.items() if v["sat"]]
        print(f"  N=12 canonical witness exists in regimes: {regimes}.")
    else:
        print(f"  No N=12 canonical witness found in any regime.")
    out = os.path.join(SCRIPT_DIR, f"n{N}_canonical_search_result.json")
