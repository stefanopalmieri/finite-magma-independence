...
| 6 | + `callcc` absorption (the continuation relation). **Architecture decision of record**: `callcc` cannot live in big-step `EvS` (a big-step judgment has no continuation to capture) — rung 6 is a **small-step simulation layer** over state pairs, reusing the same dispatch kit (which is induction-agnostic: pure `stepIter` equalities). The big-step masters keep the 12-form fragment and its corollaries. First moves: generalize the right-side inversion glue from `KRempty` to arbitrary `KR` (the kit needs nothing), design `KR` structurally over the continuation transformers, template `FactorizationCtrl.machine_sim`. The tower corollary lands here or at 7, driven by rung 5½'s completeness once META is in-fragment. | [open] |
