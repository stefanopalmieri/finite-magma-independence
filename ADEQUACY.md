...
## 0. The target theorem — **STATUS: PROVEN** (rungs 0–7 all [lean], 2026-08-06; `AdequacyTop.interpreter_adequacy`, `AdequacyControl.tower`)
...
1. **Tower collapse at every height** — apply the theorem `n` times;
   ...
   induction. **[lean — `AdequacyControl.tower`]**
2. **The interpreted world inherits every certified law** — β, the
   factorization law, call/cc behavior, store discipline — with no
   new proofs. **[lean — `AdequacyTop.law_lift` / `observable_agreement`]**
3. **The adequacy corpus (17 programs) demotes** from evidence to
   regression test. **[done by existence — every corpus behavior is an
   instance of `interpreter_adequacy`; the Rust difftest keeps the
   corpus as cross-implementation regression armor]**
4. The pearl's sequel exists. **[the theorems do; the paper is the
   remaining prose]**
...
| 5½ | `AdequacyComplete.lean` (12 thms): **completeness — a terminating run is a derivation.** `WfV`/`WfSt` (locations bounded by the store length, closure bodies control-free, no continuation values) with preservation through all 22 clauses (`evS_wf`: stores only grow) — discharging rung 5's in-bounds premise: **out-of-bounds reads are provably unreachable** from closed programs, not merely excluded. `evS_complete` by induction on fuel (`loop_through` + the ladder's `loop_mono_le` doing the bookkeeping; the deref case discharges its premise from well-formedness); `evS_complete_halt` pins the value. Headline `meta_inherits_convergence`: **the interpreted world converges whenever the direct world does**, for every control-free closed program, from the run itself — no derivation hypothesis. **Honest re-scoping discovered here**: the tower corollary cannot land at 5½ — META's image contains a (dead-branch) `callcc` form, so `Ctl0 META` is false and completeness does not apply to META's own runs; the fix is exactly rung 6's small-step layer, where `callcc` joins the relation and META becomes fully in-fragment — **the tower moves to rung 6/7** with this rung's machinery as its engine. Elaboration: seconds (pure induction — no kernel reductions). | **[lean]** |
| 6 | + `callcc` absorption. **Architecture decision of record**: `callcc` cannot live in big-step `EvS` — rung 6 is a **small-step simulation layer** over state pairs, reusing the dispatch kit unchanged. **Kit landed** (`AdequacyCtlKit.lean`, 10 thms): `meval_callcc_S` — 132 steps, tail call, the captured continuation **literally the base `κ`** (`beqK`-verified by probe, `rfl`-certified), shf-tagged and consed onto the environment: the machine's `callcc` arm one level up; `mapply_shf_S` — the throw, 260 steps, tagged value delivered *verbatim* to the target (value and target passengers): the cont-application arm one level up; seven **continuations-as-values** arms, all agreeing with the machine (`car`/`cdr`/`deref` default 57, `pairp` no 57, `ite` truthy-then 107, elem-applied-to-cont default 183, `setref`-target default 57 — the campaign's error-arm agreement streak unbroken); `meta_eqvFree` — **the frozen image is in the 13-form domain** (`eqvFreeB` + `native_decide`), the fact that lets the simulation apply to META's own runs. **The simulation design (of record, for the main file)**: (1) mutual inductive `RepVc`/`RepEnvc`/`KRel` (ρ₀-parametric) — `RepVc` = rung-2's clauses with `EqvFree b` added to `clos` (entered bodies must be interpretable) and `kont` via `KRel`; `KRel` = one clause per direct frame, meta side the corresponding transformer applied at related pieces (dead transformer parameters become constructor arguments; `refK`'s meta side is the concrete `refK·consR[E5]` frames). (2) `SRel`: eval-pairs (focus `EqvFree`, `RepEnvc` envs, `Forall₂ RepVc` store suffixes, `KRel` konts) and ret-pairs. (3) a **full well-formedness** `WfV6`/`WfK6` mutual over `Val`/`Kont` (continuations now wf-able, unlike rung 5½'s) with step-preservation — needed at the `derefK` moment (the OOB gap returns in small-step form and is discharged the same way). (4) `sim_step`: `SRel` + wf → every direct step is matched by a ≥1-step META segment preserving both (~35 cases, each discharged by a kit lemma + a `KRel` constructor; the callcc case hands the `KRel` evidence over directly since capture = base κ). (5) corollaries: multi-step simulation, **two-sided divergence transfer**, closed-program adequacy for the full **13-form domain** (= `EqvFree`, exactly META v1's), and **the tower**: the simulation relates *any* machine run to its META image — including runs that are themselves META interpreting something (`meta_eqvFree` puts META in-fragment) — so tower collapse at every height is the multi-step theorem composed with itself; no completeness detour needed. **The simulation landed** (`AdequacyEntry.lean` 1 thm + `AdequacyControl.lean` 33 thms): `KRel` pairs each direct frame with its certified transformer at related pieces — **continuation representation is the dispatch architecture**; `sim_step` matches all ~40 machine-step cases with kit segments; `WfV6`/`WfK6` + `step_wf` discharge the in-bounds read; `sim_run`/`sim_diverge` = **two-sided behavior transfer**; `adequacy_ctl` = adequacy for the **full 13-form domain, `callcc` included**; `meta_diverges` closes the divergence direction. **THE TOWER**: `tower_step` (one interpretation layer as a theorem — its hypotheses re-established by its own conclusion, so it self-composes) and `tower` (**collapse at every height**: every finite iterate `META ⬝ ⌜META ⬝ ⌜⋯⌜p⌝⋯⌝⌝` terminates — the machine rung's two-level demo is the `k = 2` instance). Elaboration: **7 seconds** (the induction is free once the kit is paid for). Axiom footprint: standard + the documented `native_decide` pair via `meta_eqvFree`. | **[lean]** |
| 7 | `AdequacyTop.lean` (4 thms): **the top theorem — the campaign closes.** `interpreter_adequacy` = §0 as stated: a **monotone fuel transformer** `F` (built by choice from `adequacy_ctl`, monotonized by running max) under which convergence within `n` direct steps becomes convergence within `F n` META steps to a representing value, with divergence transferring; `halts_iff` — **the interpreted program halts iff the program halts**; `observable_agreement` — element results are *determined* (the interpreter answers with exactly the tagged quotation, `elemRc` pins it); `law_lift` — machine-equal programs are interpreted-equal, so every certified law lifts with no new proofs. Elaboration: 1.3 s. With rung 6's `tower`, **all four corollaries of §0 are theorems**. | **[lean]** |
