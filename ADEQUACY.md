...
| 3b(iv) | The general simulation induction: adequacy for `app f x` with `f, x` universally quantified — META's internal calling convention (recursive `meval` under META's own frames), the continuation relation instantiated, per-form fuel transformers. The families of 3b(iii) are its base cases and regression net. | [open] |
