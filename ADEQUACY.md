...
| 6 | + `callcc` absorption (the continuation relation). **Architecture decision of record**: `callcc` cannot live in big-step `EvS` (a big-step judgment has no continuation to capture) — rung 6 is a **small-step simulation layer** over state pairs, reusing the same dispatch kit (which is induction-agnostic: pure `stepIter` equalities). The big-step masters keep the 12-form fragment and its corollaries. First moves: generalize the right-side inversion glue from `KRempty` to arbitrary `KR` (state fresh copies in the rung-6 file — do **not** edit the committed files, hours of re-elaboration), design `KR` structurally over the continuation transformers, template `FactorizationCtrl.machine_sim`. **Probe findings (2026-08-05, groundwork laid)**: the absorption is structurally perfect — `callcc` dispatch is **132 steps to a tail-position recursive call**, the captured continuation shf-tagged and consed onto the tagged environment (`(E4 . cont κ) :: ρT`), mirroring the machine's own `callcc` arm *at the base continuation* (so the capture point's κ is exactly the simulation's κ — the KR evidence is handed over by the invariant, no reconstruction); the **throw** works through the ordinary `mapply` protocol — applying a shf-tagged value delivers the tagged payload straight to the captured κ, the machine's cont-application arm one level up (observed end-to-end: `callcc (k 5)` returns `(quo.5)` to the base in 308 steps). Expected dispatch lemmas: `meval_callcc_S` (132, tail, captures base κ — the `rfl` will verify the capture is literally `κ`) and `mapply_shf_S` (throw segment, value rides unchanged). The tower corollary lands here or at 7, driven by rung 5½'s completeness once META is in-fragment. | [open] |
